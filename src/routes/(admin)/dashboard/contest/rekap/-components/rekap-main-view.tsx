...
export function RekapMainView({ contestId }: RekapMainViewProps) {
  ...
  const [kelompokList, setKelompokList] = React.useState<KelompokRekapData[]>([]);
  ...
  React.useEffect(() => {
    ...
    const fetchApiRekap = async () => {
      try {
        setIsLoading(true);
        const contestNumericId = parseInt(String(contestId || "1").replace(/\D/g, ""), 10) || undefined;
        ...
        if (isMounted && res.status && Array.isArray(res.data) && res.data.length > 0) {
          ...
          const validItems = detailedList.filter(
            (item): item is KelompokRekapData => item !== null,
          );
          if (validItems.length > 0 && isMounted) {
            setKelompokList(validItems);
          }
        }
      } catch (err) {
        console.warn("[Rekap API fetch warning]", err);
      } finally {
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex flex-col gap-6">
        <div className="flex items-center justify-between border-b pb-3 gap-2 flex-wrap">
          ...
          <div className="text-xs text-muted-foreground font-mono hidden md:flex items-center gap-1.5">
            ...
            <span>{kelompokList.length} Kelompok &bull; Klik Baris untuk Buka Dialog Jawaban</span>
          </div>
        </div>
        ...
        <TabsContent value="rekap" className="m-0 flex flex-col gap-8">
          ...
          <RekapOverviewCards onOpenKelompokModal={handleOpenModal} groups={kelompokList} />

          {/* Direct Cards for Groups */}
