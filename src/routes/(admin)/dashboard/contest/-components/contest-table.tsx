            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <TableHead key={header.id} className="py-3 font-semibold text-xs">
              ...
              table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <TableCell key={cell.id} className="px-4 py-2.5 align-middle text-xs">
              ...
              <TableRow>
                ...
                >
                  <div className="flex flex-col items-center justify-center gap-1.5 py-6">
                    ...
                    <span className="font-semibold text-foreground">Tidak ada data lomba</span>
                    <span className="text-[11px] text-muted-foreground">
                      Belum ada kontes lomba yang terdaftar atau sesuai kriteria pencarian.
                    </span>
                  </div>
                </TableCell>
              </TableRow>
      ...
      <div className="flex flex-col gap-4 px-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4 text-muted-foreground text-xs">
          <div className="flex items-center gap-2">
            ...
            >
              <SelectTrigger size="sm" className="w-18 h-7 text-xs" id="lomba-rows-per-page">
